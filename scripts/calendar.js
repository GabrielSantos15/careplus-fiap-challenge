...
// Variáveis globais para armazenar data e horário original da consulta (para reagendamento)
let originalConsultaDate = null;
let originalConsultaTime = null;
...
function updateConfirmState() {
  ...
  confirmButton.classList.toggle("disabled", !isReady);
}
  ...
  for (let week = 0; week < 6; week++) {
    ...
    for (let dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
      ...
      if (week === 0 && dayOfWeek < firstDayIndex) {
        ...
        // Marca o dia original da consulta (se houver) com tooltip
        if (
          originalConsultaDate &&
          dateObj.getTime() === originalConsultaDate.getTime()
        ) {
          dayDiv.classList.add("original-consulta");
          dayDiv.title = "Data original da consulta";
          dayDiv.setAttribute("data-bs-toggle", "tooltip");
          dayDiv.setAttribute("data-bs-placement", "top");
          dayDiv.setAttribute("data-bs-custom-class", "custom-tooltip");
        }
...
function selectDate(element, day, month, year) {
  ...
  updateSummary();
  updateConfirmState();
  renderTimeSlots(); // Re-renderiza os horários para mostrar/ocultar a marca do horário original
  initTooltips(); // Reinicializa tooltips
}
...
function renderTimeSlots() {
  ...
  periodo.slots.forEach((slot) => {
    ...
    // Marca o horário original apenas se o dia selecionado for o dia original da consulta
    if (
      originalConsultaTime &&
      slot.label.startsWith(originalConsultaTime) &&
      selectedDate &&
      originalConsultaDate &&
      selectedDate.getTime() === originalConsultaDate.getTime()
    ) {
      button.classList.add("original-horario");
      button.title = "Horário original da consulta";
    ...
    if (
      selectedTimeSlot &&
      selectedTimeSlot.periodKey === periodo.key &&
...
function changePeriod(direction) {
  currentPeriodIndex =
    (currentPeriodIndex + direction + periodos.length) % periodos.length;
...
// Função para atualizar a data original da consulta e re-renderizar o calendário
function updateOriginalConsultaDate(dateIso) {
  if (dateIso) {
    originalConsultaDate = new Date(dateIso);
    renderCalendar(); // Re-renderiza para marcar o dia original
    initTooltips();
  }
}

// Função para atualizar o horário original da consulta e re-renderizar
function updateOriginalConsultaTime(timeLabel) {
  if (timeLabel) {
    originalConsultaTime = timeLabel;
    renderTimeSlots(); // Re-renderiza para marcar o horário original
    initTooltips();
  }
}
